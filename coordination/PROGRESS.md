...
| T-037 | Fondo compartido #1A1038→#A78BEA en secciones 4-8 | todo | Claude | 2026-07-10 |
...
- 2026-07-10 · Claude · T-037 · Yuli pidió deshacer T-037 por completo tras dos idas y vueltas (lineal → manchas de blur), pero manteniendo intacto todo lo de T-032/T-038 (fondo Hero/IntroPortada) de la misma tanda de pedidos. Confirmado por pregunta directa: se revierte el fondo de las 5 secciones a como estaba antes de T-037 (`PhasesDetail`/`AboutMe`/`Testimonios` → `transparent`; `Sistema`/`QualificationSections` → `#281a52` sólido) y se elimina `--gradient-section` de `styles.css`. `IntroPortada.tsx`/`Hero.tsx` no se tocan. T-037 vuelve a `todo` (no implementada) → si se retoma, aclarar de entrada qué versión de fondo (lineal vs. manchas) antes de commitear.
