...
- 2026-07-10 · Claude · T-037,T-038 · Nuevo token `--gradient-section` (`#1A1038`→`#A78BEA`, 145deg) aplicado como fondo uniforme en PhasesDetail/Sistema/QualificationSections/AboutMe/Testimonios (secciones 4-8), reemplazando la mezcla de sólidos `#281a52`/transparent que había. — `IntroPortada.tsx` reestructurado con el patrón de `CaosToSistema.tsx` (wrapper 180vh + marco `position:sticky` 100vh) para que la capa de degradé sobre el video se pinee, crezca hasta cubrir todo el marco pasada la mitad del recorrido, y entregue el control al Hero (que arranca con el mismo color `#F0ECFF`) sin salto visible — antes la capa scrolleaba con la página normal y podía quedar mal posicionada/tapar el logo. Texto/botón de la primera pantalla ahora se desvanecen antes de que la capa cubra el video. Botón "Conocer" hace scroll hasta el final real del wrapper → **pendiente crítico**: verificar en `bun run dev` que el timing (umbral 0.5, alto 180vh) y el fade del navbar de Hero (que arranca ~80vh más tarde que antes al moverse el offset) no se pisen entre sí; ajustar a ojo si hace falta.
